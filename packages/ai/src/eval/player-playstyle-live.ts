...
import type { GeneratePlayerPlaystyleConfig } from '../generation/generate-player-playstyle';
...
import type { PlayerPlaystyleEvalFixture } from './player-playstyle-fixture-schema';
...
export type PlayerPlaystyleLiveEvalMetrics = {
  fixtures: number;
...
export type PlayerPlaystyleLiveEvalResult = {
  exitCode: number;
  skipped: boolean;
  metrics: PlayerPlaystyleLiveEvalMetrics | null;
};
...
function readGenerationConfig(env: NodeJS.ProcessEnv): GeneratePlayerPlaystyleConfig {
  return {
...
function emptyMetrics(model: string, fixtures: number): PlayerPlaystyleLiveEvalMetrics {
  return {
    fixtures,
...
function validationCause(error: unknown): PlayerPlaystyleValidationError | undefined {
  if (error instanceof PlayerPlaystyleValidationError) {
    return error;
  }
  if (
    error instanceof AiOutputValidationError &&
    error.cause instanceof PlayerPlaystyleValidationError
  ) {
    return error.cause;
  }
  return undefined;
...
function recordValidationFailure(metrics: PlayerPlaystyleLiveEvalMetrics, error: unknown): void {
...
function recordProviderContent(
  metrics: PlayerPlaystyleLiveEvalMetrics,
  wrapped: {
...
function printMetrics(metrics: PlayerPlaystyleLiveEvalMetrics, write: EvalWriter): void {
...
export async function runPlayerPlaystyleLiveEval(options?: {
  env?: NodeJS.ProcessEnv;
  provider?: AiProvider;
  fixtures?: PlayerPlaystyleEvalFixture[];
  fixturesDir?: string;
  write?: EvalWriter;
}): Promise<PlayerPlaystyleLiveEvalResult> {
  ...
  for (const fixture of fixtures) {
    ...
    try {
      ...
      if (
        error instanceof AiOutputValidationError ||
        error instanceof PlayerPlaystyleValidationError
      ) {
        recordProviderContent(metrics, wrapped);
        ...
        if (cause) {
          write(`kind=${cause.code} reason=${cause.details.reason}`);
        }
